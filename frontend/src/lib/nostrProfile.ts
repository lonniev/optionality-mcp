// Nostr kind-0 profile — DRY edition. Relay I/O lives in the wheel
...
//
// `window.nostr` (NIP-07) is typed globally by ./nostr — we reuse that.
...
export async function publishProfile(content: Kind0): Promise<PublishNostrProfileResult> {
  ...
  if (hasSessionNsec()) {
    signedJson = JSON.stringify(finalizeEvent(template, getSessionNsecBytes()));
  } else if (typeof window !== "undefined" && window.nostr) {
    const pubkey = await window.nostr.getPublicKey();
    signedJson = JSON.stringify(await window.nostr.signEvent({ ...template, pubkey }));
  } else {
    throw new Error(
  ...
  return publishNostrProfile(getStoredNpub(), signedJson);
}
